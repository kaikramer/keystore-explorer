	...
	private void colAdjust(JTable jtKeyStore)
	{
		...
		for (int i = 0; i < jtKeyStore.getColumnCount(); i++) {
			...
			if (i == keyStoreTableColumns.colEntryName())
			{
				...
				column.setPreferredWidth(Math.max(1+res.getString("KeyStoreTableModel.NameColumn").length(),20)*iFontSize);
				column.setMaxWidth(Math.max(2+res.getString("KeyStoreTableModel.NameColumn").length(),50)*iFontSize);
			}
			if (i == keyStoreTableColumns.colAlgorithm())
			{
				...
				column.setMaxWidth(Math.max(2+res.getString("KeyStoreTableModel.AlgorithmColumn").length(),5)*iFontSize);
			}
			...
			if (i == keyStoreTableColumns.colCurve())
			{
				...
				column.setMaxWidth(2+Math.max("brainpool999r1".length(),res.getString("KeyStoreTableModel.CurveColumn").length())*iFontSize);
			}
