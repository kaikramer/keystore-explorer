...
import static org.kse.crypto.x509.ExtendedKeyUsageType.TLS_SIGNING;
...
public class DExtendedKeyUsage extends DExtension {
	...
	private JCheckBox jcbTlsSigning;
	...
	private void initComponents() {
		...
		jcbTlsSigning = new JCheckBox(res.getString("DExtendedKeyUsage.jcbTlsSigning.text"));
		...
		pane.add(jcbTlsSigning, "wrap unrel");
	...
	private void prepopulateWithValue(byte[] value) throws IOException {
		...
		for (KeyPurposeId keyPurposeId : extendedKeyUsage.getUsages()) {
			...
			} else if (type == TLS_SIGNING) {
				jcbTlsSigning.setSelected(true);			
	...
	private void okPressed() {
		if (!jcbTlsWebServerAuthentication.isSelected() && !jcbTlsWebClientAuthentication.isSelected()
				...
				&& !jcbDocumentSigning.isSelected() && !jcbAdobePDFSigning.isSelected() && !jcbTlsSigning.isSelected()
		...
		if (jcbTlsSigning.isSelected()) {
			keyPurposeIds.add(KeyPurposeId.getInstance(new ASN1ObjectIdentifier(TLS_SIGNING.oid())));
		}
