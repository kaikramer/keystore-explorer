...
public class ImportCaReplyFromFileAction extends AuthorityCertificatesAction implements HistoryAction {
    ...
    public ImportCaReplyFromFileAction(KseFrame kseFrame) {
        ...
        putValue(SMALL_ICON, new ImageIcon(
                Toolkit.getDefaultToolkit().createImage(getClass().getResource("images/importcareplyfile.png"))));
    }
            ...
            if (!applicationSettings.getEnableImportCaReplyTrustCheck()) {
                ...
                if (certs.length > 1) {
                    ...
                    if (matchAlias == null) {
                        ...
                        DViewCertificate dViewCertificate = new DViewCertificate(frame, MessageFormat.format(
                                ...
                                caReplyFile.getName()), new X509Certificate[] { rootCert }, null,
                                                                                 DViewCertificate.NONE);
                        dViewCertificate.setLocationRelativeTo(frame);
                        dViewCertificate.setVisible(true);

                        int selected = JOptionPane.showConfirmDialog(frame, res.getString(
                                "ImportCaReplyFromFileAction.AcceptCaReply.message"), res.getString(
                                "ImportCaReplyFromFileAction.ImportCaReply.Title"), JOptionPane.YES_NO_OPTION);
                ...
                // trust from the certificate and ending with a root CA self-signed certificate
                    ...
                    if (trustChain != null) {
                        ...
                        JOptionPane.showMessageDialog(frame, res.getString(
                                                              "ImportCaReplyFromFileAction.NoTrustCaReply.message"),
                                                      res.getString("ImportCaReplyFromFileAction.ImportCaReply.Title"),
                                                      JOptionPane.WARNING_MESSAGE);
                        return;
                    }
        ...
        try {
            ...
            if (certs.length == 0) {
                JOptionPane.showMessageDialog(frame, MessageFormat.format(
                                                      res.getString("ImportCaReplyFromFileAction.NoCertsFound.message"), caReply),
            ...
            DProblem dProblem = new DProblem(frame,
                                             res.getString("ImportCaReplyFromFileAction.ProblemOpeningCaReply.Title"),
