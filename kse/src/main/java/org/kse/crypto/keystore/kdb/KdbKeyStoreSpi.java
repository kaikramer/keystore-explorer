...
/**
 * A KeyStore service provider implementation backed by an IBM CMS key database ({@code .kdb})
 * as used by IBM WebSphere, IBM HTTP Server and DB2. "CMS" here is IBM's Certificate
 * Management System, unrelated to the RFC 5652 Cryptographic Message Syntax.
 *
...
public class KdbKeyStoreSpi extends KeyStoreSpi {
    ...
    public void engineLoad(InputStream stream, char[] password)
        ...
        if (!KdbKeyDatabase.isKeyDatabase(data)) {
            throw new IOException("Not an IBM CMS key database (bad magic/type tag)");
        }

        if (password != null && !KdbKeyDatabase.verify(data, password)) {
            throw new IOException("Password verification failed",
                                  new UnrecoverableKeyException("IBM CMS key database password incorrect"));
        }
    ...
    public void engineSetKeyEntry(String alias, Key key, char[] password, Certificate[] chain)
        ...
        if (chain == null || chain.length == 0 || !(chain[0] instanceof X509Certificate)) {
            throw new java.security.KeyStoreException(
                    "A IBM CMS key database entry requires an X.509 certificate for the private key");
        }
        try {
            byte[] encryptedKey = Pbes2.encrypt((PrivateKey) key, password);
    ...
    public void engineSetKeyEntry(String alias, byte[] key, Certificate[] chain)
            throws java.security.KeyStoreException {
        throw new java.security.KeyStoreException(
                "Storing already protected keys is not supported by IBM CMS key databases");
    }
