...
public class Spkac {
     ...
     * @throws SpkacException If construction fails
     */
    ...
    public Spkac(byte[] data) throws IOException, SpkacException {
        ...
        if (!properties.containsKey(SPKAC_PROPERTY)) {
            throw new SpkacMissingPropertyException(
                    MessageFormat.format(res.getString("SpkacNoIncludeRequiredProperty.exception.message"),
                                         SPKAC_PROPERTY));
        }
    ...
    private SignatureType getSignatureAlgorithm(ASN1ObjectIdentifier signatureAlgorithmOid) throws SpkacException {
        ...
        if (sigAlg == null) {
            throw new SpkacException(
                    MessageFormat.format(res.getString("NoSupportSignatureAlgorithm.exception.message"),
                                         signatureAlgorithmOid.getId()));
        }
    ...
    private PublicKey decodePublicKeyFromBitString(ASN1ObjectIdentifier publicKeyAlgorithmOid,
                                                   ASN1Primitive algorithmParameters, DERBitString publicKey)
        ...
        if (publicKeyAlgorithmOid.getId().equals(RSA.oid())) {
            ...
            throw new SpkacException(
                    MessageFormat.format(res.getString("NoSupportPublicKeyAlgorithm.exception.message"),
                                         publicKeyAlgorithmOid.getId()));
     ...
     * @throws SpkacException If verification fails
     */
