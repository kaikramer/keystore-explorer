...
public class X509Ext {
    ...
    private static String getSubjectKeyIdentifierStringValue(byte[] value) throws IOException {
        ...
        sb.append(
                MessageFormat.format(res.getString("SubjectKeyIdentifier"), HexUtil.getHexString(keyIdentifierBytes)));
        sb.append(NEWLINE);
    ...
    private static String getAuthorityKeyIdentifierStringValue(byte[] value) throws IOException {
        ...
        if (keyIdentifier != null) { // Optional
            ...
            sb.append(
                    MessageFormat.format(res.getString("AuthorityKeyIdentifier"), HexUtil.getHexString(keyIdentifier)));
            sb.append(NEWLINE);
        }
    ...
    private static String getReasonCodeStringValue(byte[] value) throws IOException {
        ...
        sb.append(MessageFormat.format(resCryptoX509.getString("CrlReason.format"), text, crlReasonLong));
    ...
    private static String getHoldInstructionCodeStringValue(byte[] value) throws IOException {
        ...
        HoldInstructionCodeType holdInstructionCodeType = HoldInstructionCodeType.resolveOid(
                holdInstructionCode.getId());
            ...
            if (policyQualifiers != null) { // Optional
                ...
                for (ASN1Encodable policyQualifier : policyQualifiers.toArray()) {
                    ...
                    CertificatePolicyQualifierType certificatePolicyQualifierType =
                            CertificatePolicyQualifierType.resolveOid(
    ...
    private static String getGeneralizedTimeString(ASN1GeneralizedTime asn1GeneralizedTime) {
        try {
            return StringUtils.formatDate(asn1GeneralizedTime.getDate());
        } catch (ParseException e) {
            throw new IllegalArgumentException("Cannot parse date");
        }
    }

    private static String getUtcTimeString(ASN1UTCTime asn1UTCTime) {
        try {
            return StringUtils.formatDate(asn1UTCTime.getDate());
        } catch (ParseException e) {
            throw new IllegalArgumentException("Cannot parse date");
        }
    }
        ...
        for (Admissions admission : admissions) {
            ...
            for (ProfessionInfo professionInfo : professionInfos) {
                ...
                for (DirectoryString professionItem : professionItems) {
                    sb.append(INDENT.toString(indentLevel));
                    sb.append(
                            MessageFormat.format(res.getString("Admission.ProfessionItem"), professionItem.toString()));
                    sb.append(NEWLINE);
                }
                ...
                if (professionOIDs != null) {
                    for (ASN1ObjectIdentifier professionOID : professionOIDs) {
                        sb.append(INDENT.toString(indentLevel));
                        sb.append(
                                MessageFormat.format(res.getString("Admission.ProfessionOID"), professionOID.getId()));
                        sb.append(NEWLINE);
                    }
                }
    ...
    private static String getMsCrlNextPublishStringValue(byte[] octets) throws IOException {
        ...
        ASN1Primitive asn1Primitive = ASN1Primitive.fromByteArray(octets);
        if (asn1Primitive instanceof ASN1GeneralizedTime) {
            return getGeneralizedTimeString((ASN1GeneralizedTime) asn1Primitive);
        } else {
            return getUtcTimeString((ASN1UTCTime) asn1Primitive);
        }
    }
