...
public class MsPvkUtil {
    ...
    public static PrivateKey load(byte[] pvk) throws IOException, CryptoException {
        ...
        if (encrypted != PVK_UNENCRYPTED) {
            throw new PrivateKeyEncryptedException(
                    MessageFormat.format(res.getString("MsPvkIsEncrypted.exception.message"),
                                         Long.toHexString(encrypted), Long.toHexString(PVK_UNENCRYPTED)));
        }
        ...
        if (keyLength != (privateKeyBlob.length + BLOB_HEADER_LENGTH)) {
            throw new CryptoException(
                    MessageFormat.format(res.getString("InvalidMsPvkKeyLengthField.exception.message"),
                                         Long.toHexString(keyLength),
    ...
    public static PrivateKey loadEncrypted(byte[] pvk, Password password) throws IOException, CryptoException {
        try {
            ...
            if (encrypted != PVK_ENCRYPTED) {
                throw new PrivateKeyUnencryptedException(
                        MessageFormat.format(res.getString("MsPvkIsUnencrypted.exception.message"),
                                             Long.toHexString(encrypted), Long.toHexString(PVK_ENCRYPTED)));
            }
            ...
            if (saltLength != ENCRYPTED_SALT_LENGTH) { // Specific length for encrypted PVK
                throw new CryptoException(
                        MessageFormat.format(res.getString("InvalidMsPvkSaltLengthField.exception.message"),
                                             Long.toHexString(saltLength), Long.toHexString(ENCRYPTED_SALT_LENGTH)));
            }
     ...
     * @param keyType    PVK_KEY_EXCHANGE or PVK_KEY_SIGNATURE
     * @return The encoding
     ...
     * @param strong     Use strong key for encryption?
     * @return The encoding
     * @throws CryptoException Problem encountered while getting the encoded private key
     */
    ...
    private static void writeReservedMagicKeyType(ByteBuffer bb, long keyType) throws IOException, CryptoException {
        ...
        if ((keyType != PVK_KEY_EXCHANGE) && (keyType != PVK_KEY_SIGNATURE)) {
            throw new CryptoException(MessageFormat.format(res.getString("InvalidMsPvkKeyTypeField.exception.message"),
                                                           ...
                                                           Long.toHexString(PVK_KEY_EXCHANGE),
                                                           Long.toHexString(PVK_KEY_SIGNATURE)));
        }
    ...
    private static long readReservedMagicKeyType(ByteBuffer bb) throws CryptoException {
        ...
        if (magic != PVK_MAGIC_NUMBER) {
            throw new CryptoException(MessageFormat.format(res.getString("InvalidMsPvkMagicField.exception.message"),
                                                           ...
                                                           Long.toHexString(PVK_MAGIC_NUMBER)));
        }

        long reserved = UnsignedUtil.getInt(bb);

        if (reserved != PVK_RESERVED) {
            throw new CryptoException(MessageFormat.format(res.getString("InvalidMsPvkReservedField.exception.message"),
                                                           Long.toHexString(reserved), Long.toHexString(PVK_RESERVED)));
        }
    ...
    private static void readPrivateKeyBlobHeader(ByteBuffer bb, long keyType) throws CryptoException {
        ...
        if (blobType != PRIVATE_KEY_BLOB) {
            throw new CryptoException(
                    MessageFormat.format(res.getString("InvalidBlobHeaderTypeField.exception.message"),
                                         Integer.toHexString(blobType), Integer.toHexString(PRIVATE_KEY_BLOB)));
        }
        ...
        if (blobVersion != CUR_BLOB_VERSION) {
            throw new CryptoException(
                    MessageFormat.format(res.getString("InvalidBlobHeaderVersionField.exception.message"),
                                         Integer.toHexString(blobVersion), Integer.toHexString(CUR_BLOB_VERSION)));
        }
        ...
        if (blobReserved != BLOB_RESERVED) {
            throw new CryptoException(
                    MessageFormat.format(res.getString("InvalidBlobHeaderReservedField.exception.message"),
        ...
        if (keyType == PVK_KEY_SIGNATURE) {
            ...
            if ((keyAlgId != CALG_RSA_SIGN) && (keyAlgId != CALG_DSS_SIGN)) {
                throw new CryptoException(
                        MessageFormat.format(res.getString("InvalidBlobHeaderKeyAlgIdField.exception.message"),
                                             Long.toHexString(keyAlgId), Long.toHexString(CALG_RSA_SIGN),
                                             Long.toHexString(CALG_DSS_SIGN)));
            }
        } else {
            ...
            if (keyAlgId != CALG_RSA_KEYX) {
                throw new CryptoException(
                        MessageFormat.format(res.getString("InvalidBlobHeaderKeyAlgIdField.exception.message"),
                                             Long.toHexString(keyAlgId), Long.toHexString(CALG_RSA_KEYX)));
            }
        }
    }
        ...
        try {
            ...
            if (magic != RSA_PRIV_MAGIC) {
                throw new CryptoException(MessageFormat.format(res.getString("InvalidRsaMagicField.exception.message"),
                                                               Long.toHexString(magic),
                                                               Long.toHexString(RSA_PRIV_MAGIC)));
            }
        ...
        try {
            ...
            if (magic != DSS_PRIV_MAGIC) {
                throw new CryptoException(MessageFormat.format(res.getString("InvalidDsaMagicField.exception.message"),
                                                               Long.toHexString(magic),
                                                               Long.toHexString(DSS_PRIV_MAGIC)));
            }
