...
fn get_java_exe_path() -> String {
    let local_jre_path = get_absolute_parent_path().join("jre");
    if local_jre_path.is_dir() {
        format!("{}\\bin\\javaw.exe", local_jre_path.display())
    } else {
...
fn main() {
    ...
    let java_path = get_java_exe_path();
    ...
    if let Err(e) = java_process {
        show_error_message(
            CString::new("Error").unwrap(),
            CString::new(format!("Error starting Java runtime:\n{}", e)).unwrap());
    }
}
